 ...
 * 
 * ```
 * Flog(INFO, "Log with no tag to the default channel at level %s", "INFO");
 * Flogt(WARN, "foo", "Log with %s tag to the default channel at level %s", "foo", "WARN");
 * Flogtc(DBUG, "", 1, "Log with no tag to channel %d at level %s", 1, "DBUG");
 * ```
 * 
